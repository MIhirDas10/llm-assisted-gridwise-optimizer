...
For every other directive, applies must be true. Time windows are start-inclusive and end-exclusive. The end boundary is not an affected hour. Convert all time expressions to unique ascending integer hours from 0 through 23. Examples:
...
Recognize paraphrases by meaning, not by exact words. Apply the stated whole-hour boundaries exactly and never extend a window through its end boundary. Do not invent missing hours, values, or unsupported directive types. Preserve kWh units exactly. Explanations must be short and must not mention the model or prompt."""
